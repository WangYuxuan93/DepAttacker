...
def read_data(source_paths: [str], word_alphabet: Alphabet, char_alphabet: Alphabet, 
            pos_alphabet: Alphabet, type_alphabet: Alphabet, pre_alphabet=None, max_size=None, 
            normalize_digits=True, symbolic_root=True, symbolic_end=False,
            pos_idx=4, prior_order='inside_out'):
    data = []
    max_length = 0
    max_char_length = 0
    counter = 0
    ...
    for set_id, source_path in enumerate(source_paths):
        ...
        while inst is not None and (not max_size or counter < max_size):
            ...
            data.append([sent.word_ids, sent.char_id_seqs, inst.pos_ids, inst.heads, inst.type_ids, sent.pre_ids, stacked_heads, children, siblings, stacked_types, skip_connect])
    ...
    for i, inst in enumerate(data):
        wids, cid_seqs, pids, hids, tids, preids, stack_hids, chids, ssids, stack_tids, skip_ids = inst
        inst_size = len(wids)
        lengths[i] = inst_size
        # word ids
    ...
    data_tensor = {'WORD': words, 'CHAR': chars, 'POS': pos, 'HEAD': heads, 'TYPE': types, 'MASK_ENC': masks_e,
                   ...
                   'PRETRAINED': pres, 'SRC': np.array(src_words), 'ERR_TYPE': np.array(err_types)}
...
def read_bucketed_data(source_paths: [str], word_alphabet: Alphabet, char_alphabet: Alphabet, pos_alphabet: Alphabet, type_alphabet: Alphabet,
                       ...
                       mask_out_root=False, pos_idx=4, prior_order='inside_out'):
        ...
        while inst is not None and (not max_size or counter < max_size):
            ...
            for bucket_id, bucket_size in enumerate(_buckets):
                if inst_size < bucket_size:
                    stacked_heads, children, siblings, stacked_types, skip_connect = _generate_stack_inputs(inst.heads, inst.type_ids, prior_order)
                    data[bucket_id].append([sent.word_ids, sent.char_id_seqs, inst.pos_ids, inst.heads, inst.type_ids, sent.pre_ids, stacked_heads, children, siblings, stacked_types, skip_connect])
    ...
    for bucket_id in range(len(_buckets)):
        ...
        data_tensor = {'WORD': words, 'CHAR': chars, 'POS': pos, 'HEAD': heads, 'TYPE': types, 'MASK_ENC': masks_e,
                       'SINGLE': single, 'LENGTH': lengths, 'STACK_HEAD': stacked_heads, 'CHILD': children,
                       'SIBLING': siblings, 'STACK_TYPE': stacked_types, 'SKIP_CONNECT': skip_connect, 'MASK_DEC': masks_d,
                       'PRETRAINED': pres, 'SRC': np.array(src_words[bucket_id]), 'ERR_TYPE': np.array(err_types[bucket_id])}
